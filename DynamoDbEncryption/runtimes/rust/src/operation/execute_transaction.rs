...
impl ExecuteTransaction {
    ...
    > {
                let inner_input = crate::conversions::execute_transaction::_execute_transaction_input::to_dafny(input);
        let inner_result =
            ::dafny_runtime::md!(client.dafny_client.clone()).ExecuteTransaction(&inner_input);
        if matches!(
            inner_result.as_ref(),
            crate::r#_Wrappers_Compile::Result::Success { .. }
        ) {
            Ok(
                crate::conversions::execute_transaction::_execute_transaction_output::from_dafny(inner_result.value().clone()),
            )
        } else {
...
pub(crate) mod _execute_transaction_output;

pub(crate) mod _execute_transaction_input;
