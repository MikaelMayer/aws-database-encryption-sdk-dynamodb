...
impl DescribeGlobalTableSettings {
    ...
    > {
                let inner_input = crate::conversions::describe_global_table_settings::_describe_global_table_settings_input::to_dafny(input);
        let inner_result =
            ::dafny_runtime::md!(client.dafny_client.clone()).DescribeGlobalTableSettings(&inner_input);
        if matches!(
            inner_result.as_ref(),
            crate::r#_Wrappers_Compile::Result::Success { .. }
        ) {
            Ok(
                crate::conversions::describe_global_table_settings::_describe_global_table_settings_output::from_dafny(inner_result.value().clone()),
            )
        } else {
...
pub(crate) mod _describe_global_table_settings_output;

pub(crate) mod _describe_global_table_settings_input;
