// Copyright Amazon.com Inc. or its affiliates. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0
// Do not modify this file. This file is machine generated, and any changes to it will be overwritten.
/// Orchestration and serialization glue logic for `ImportTable`.
#[derive(::std::clone::Clone, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ImportTable;
impl ImportTable {
    /// Creates a new `ImportTable`
    pub fn new() -> Self {
        Self
    }
    pub(crate) async fn send(
        client: &crate::client::Client,
        input: crate::operation::import_table::ImportTableInput,
    ) -> ::std::result::Result<
        crate::operation::import_table::ImportTableOutput,
        crate::types::error::Error,
    > {
                let inner_input = crate::conversions::import_table::_import_table_input::to_dafny(input);
        let inner_result =
            ::dafny_runtime::md!(client.dafny_client.clone()).ImportTable(&inner_input);
        if matches!(
            inner_result.as_ref(),
            crate::r#_Wrappers_Compile::Result::Success { .. }
        ) {
            Ok(
                crate::conversions::import_table::_import_table_output::from_dafny(inner_result.value().clone()),
            )
        } else {
            Err(crate::conversions::error::from_dafny(
                inner_result.error().clone(),
            ))
        }
    }
}

pub use crate::operation::import_table::_import_table_output::ImportTableOutput;

pub use crate::operation::import_table::_import_table_input::ImportTableInput;

pub(crate) mod _import_table_output;

pub(crate) mod _import_table_input;

/// Builders
pub mod builders;
