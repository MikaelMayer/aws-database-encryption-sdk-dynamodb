...
impl DeleteItem {
    ...
    > {
                let inner_input = crate::conversions::delete_item::_delete_item_input::to_dafny(input);
        let inner_result =
            ::dafny_runtime::md!(client.dafny_client.clone()).DeleteItem(&inner_input);
        if matches!(
            inner_result.as_ref(),
            crate::r#_Wrappers_Compile::Result::Success { .. }
        ) {
            Ok(
                crate::conversions::delete_item::_delete_item_output::from_dafny(inner_result.value().clone()),
            )
        } else {
...
pub(crate) mod _delete_item_output;

pub(crate) mod _delete_item_input;
